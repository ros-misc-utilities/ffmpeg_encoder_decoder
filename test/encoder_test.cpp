...
private:
  void decodePacket(const FFMPEGPacket & msg)
  {
    ...
    if (!decoder_.isInitialized()) {
      decoder_.initialize(msg.encoding, std::bind(&DecoderTester::imageCallback, this, _1, _2), "");
    }
...
TEST(ffmpeg_encoder_decoder, encoder_msg)
{
  ...
  EXPECT_EQ(tester.getPacketSum(), 115852);
...
TEST(ffmpeg_encoder_decoder, encoder_decoder_msg)
{
  ...
  EXPECT_EQ(enc_tester.getPacketSum(), 266674);
